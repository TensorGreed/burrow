...
pub struct Limits {
    ...
    /// (#175), and what it cannot interrupt there is decoding and lexing one content stream,
    /// linear in its decoded size: about 0.9 s natively for a 250 MiB stream, just under qpdf's
    /// per-filter decode ceiling, measured in ADR 0029's #175 amendment.
    ///
