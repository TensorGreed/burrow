...
use burrow_engines::pdfsyntax::geometry::{
    Form, GlyphMetrics, Matrix, Rect as GeometryRect, Refusal, Resources, WritingMode, glyphs_in,
};
...
///
/// LibreOffice 25.x was asked for a vertically-written Japanese paragraph
...
///
/// The bytes here are burrow's own, not LibreOffice's. `tests/conformance/PROVENANCE.md` states
/// that every committed fixture is generated by this repository and carries no third-party
/// content; the real output embeds a Noto CJK subset and would break both rows. So the
/// *shape* is reproduced and the *provenance* is kept.
///
/// # How to retake the measurement
///
/// `libreoffice --headless --convert-to pdf` over a flat-ODF document whose paragraph and page
/// layout both carry `style:writing-mode="tb-rl"` and a CJK font, then read the content stream.
/// Taken 2026-09-21 against LibreOffice 25.x on Linux. Recorded because the fourth condition in
/// ADR 0029 rests on it, and a measurement nobody can retake is an assertion.
///
