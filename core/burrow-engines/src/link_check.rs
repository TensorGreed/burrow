...
//!
//! Compiled only under the `native-engines` feature on Linux, and only in test builds. It
//! exists to answer three questions that nothing else does:
//!
...
//!    check is by **sha256 of the mapped file**, not by path: `build.rs` places a
//!    verified copy beside the test binaries (because cargo does not propagate rpaths to
//!    dependent crates), so the loader may legitimately map a copy. Hashing proves it is
//!    the right library wherever it sits, which is the guarantee we actually want.
//!
//! # What changed in M1 PR 2
//!
//! This module used to declare PDFium's FFI and call `FPDF_InitLibrary` behind its own
//! `Once`. Both now live in [`crate::pdfium`]: the declarations in `pdfium::ffi`, and
//! init on the engine thread, which is the **only** thread PDFium is ever called from.
//! The obligation that made the `Once` necessary — that exactly one path in the process
//! initialises the library — is now discharged structurally rather than by convention,
//! and this module goes through the same door as everything else.

use std::ffi::{CStr, c_char};

use crate::pdfium::{destroy_library_symbol, thread};
...
/// want to prove — while actually calling it would risk tearing PDFium down under the
/// engine thread.
fn pdfium_destroy_symbol_resolves() {
    let f: unsafe extern "C" fn() = destroy_library_symbol();
...
fn qpdf_version() -> burrow_types::Result<String> {
...
#[must_use]
const fn pinned_pdfium_path() -> &'static str {
...
/// fetched and built.
#[must_use]
const fn pinned_qpdf_version() -> &'static str {
...
#[must_use]
const fn pinned_pdfium_sha256() -> &'static str {
...
fn mapped_files_containing(needle: &str) -> burrow_types::Result<Vec<String>> {
...
mod tests {
    ...
    fn pdfium_links_and_initialises_cleanly() {
        ...
        assert_eq!(
            thread::ensure_started().expect("the pdfium engine should start"),
            0,
