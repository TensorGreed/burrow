...
//!
//! The constants and the mapping both live here rather than in `pdfium::ffi`, so the web
...
//!
//! The codes themselves are `fpdfview.h:605-614`. They are plain integers: nothing here
//! links against PDFium or needs it present.
...
pub(crate) fn unreadable_page_tree() -> Error {
...
/// `pdfium::ffi::load_mem_document64` returned alongside it, not a separately fetched
/// one. (Not a rustdoc link: that module only exists when the engines are linked, so a
/// link would break the docs built without them.)
///
...
pub(crate) fn map_failure(code: c_ulong) -> Error {
    match code {
        ...
        FPDF_ERR_SUCCESS => Error::Internal(
            "pdfium reported a failure with no error code; engine state is unknown".to_owned(),
        ),

        FPDF_ERR_UNKNOWN | FPDF_ERR_FILE | FPDF_ERR_FORMAT => {
        ...
        FPDF_ERR_PASSWORD => Error::PasswordRequired,

        FPDF_ERR_SECURITY => {
            Error::Unsupported("pdfium: unsupported document security scheme".to_owned())
        }

        FPDF_ERR_PAGE => Error::Malformed("pdfium: page not found or content error".to_owned()),
        ...
        FPDF_ERR_XFALOAD | FPDF_ERR_XFALAYOUT => {
...
mod tests {
    ...
    fn a_zero_error_code_on_a_failed_call_is_never_success() {
        let err = map_failure(FPDF_ERR_SUCCESS);
        assert!(
    ...
    fn every_declared_code_maps_to_its_variant() {
        let cases = [
            (FPDF_ERR_UNKNOWN, "Malformed"),
            (FPDF_ERR_FILE, "Malformed"),
            (FPDF_ERR_FORMAT, "Malformed"),
            (FPDF_ERR_PASSWORD, "PasswordRequired"),
            (FPDF_ERR_SECURITY, "Unsupported"),
            (FPDF_ERR_PAGE, "Malformed"),
            (FPDF_ERR_XFALOAD, "Unsupported"),
            (FPDF_ERR_XFALAYOUT, "Unsupported"),
        ];
