...
fn main() {
    ...
    // THE FIRST REAL-WORLD DOCUMENTS IN THIS CORPUS SINCE #112, and they live in the
    // redaction corpus rather than in `fixtures/`. `tools/make-producer-fixtures.py` builds
    // them with LibreOffice, pdfTeX and tesseract; `tests/redaction/PROVENANCE.md` records the
    // exact producer versions. They are committed, not regenerated in CI, because neither
    // toolchain is installed there.
    //
    // They are listed here and nowhere else in this file because they are NOT this example's
    // output: it reads them to record their digest, the same arrangement `xref-bomb.pdf` has.
    // Their expected outcomes are hand-written like every other entry -- one page in, one page
    // out, turned to 90, reordered to the identity, and compressible. `split` is absent
    // deliberately: a one-page document has no cut, so there is nothing to claim.
    //
    // #112 is why they are here at all. An ordinary course PDF found a defect in minutes that
    // twenty-two synthetic cases never touched, because none of them had an embedded subset
    // font. These three do, and one of them is a scan with an invisible OCR text layer.
    for (name, file) in [
        ("real-writer", "producer-writer.pdf"),
        ("real-latex", "producer-latex.pdf"),
        ("real-ocr-scan", "producer-ocr-scan.pdf"),
    ] {
        let relative = format!("../redaction/fixtures/{file}");
        let path = dir.join(&relative);
        let bytes = std::fs::read(&path).unwrap_or_else(|e| {
            panic!(
                "reading {}: {e}. This fixture is generated by \
                 tools/make-producer-fixtures.py and committed -- see tests/redaction/PROVENANCE.md",
                path.display()
            )
        });
        println!(
            "listed {} ({} bytes, generated by tools/make-producer-fixtures.py)",
            path.display(),
            bytes.len()
        );
        cases.push(Case {
