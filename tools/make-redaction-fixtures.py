#!/usr/bin/env python3
"""Build the redaction corpus's HAND-BUILT fixtures: one per place a page's text can survive.

    python3 tools/make-redaction-fixtures.py <output-dir>

PORTED FROM spike 0006 (`spikes/redaction-survival/make-fixtures.py`), unchanged in substance.
The spike measured with these; the corpus asserts ADR 0029's verdicts with them, and
`tests/redaction/manifest.toml` says which verdict each one carries.

NOT COMMITTED, unlike the producer fixtures beside them. These are deterministic and need no
toolchain, so generating them is cheaper and more honest than storing them. The producer
fixtures are the opposite -- see `tools/make-producer-fixtures.py` and `PROVENANCE.md`.
