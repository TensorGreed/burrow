...
const ORIGIN = process.env.BURROW_SITE ?? "http://localhost:4321";

// The four files the worker loads. `.js` is Emscripten glue, `.wasm` is the module.
const ENGINE_FILES = [
  { id: "pdfiumJs", from: "pdfium.js", kind: "js", source: "engines" },
  { id: "pdfiumWasm", from: "pdfium.wasm", kind: "wasm", source: "engines" },
  { id: "qpdfJs", from: "qpdf.js", kind: "js", source: "engines" },
  { id: "qpdfWasm", from: "qpdf.wasm", kind: "wasm", source: "engines" },
  // burrow's own module is staged the same way and fetched the same way. It is not an
  // "engine", but it is a .wasm the worker has to fetch, and connect-src governs that
  // fetch identically -- so leaving it out would mean either a looser policy or a module
  // that cannot load.
  { id: "burrowJs", from: "burrow_wasm.js", kind: "js", source: "pkg" },
...
async function main() {
  ...
  const metaPolicy = policyFor(wasmUrls, { forMeta: true });
  const headerPolicy = policyFor(wasmUrls, { forMeta: false });
