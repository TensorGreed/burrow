...
| [M1](#m1--core-operations-and-the-web-app) | Merge, split, rotate, reorder, compress — core + web | in progress; `merge` ✅, `rotate` ✅, `reorder` core + bridge ✅ (page next), `split` **held** (#54), `compress` next. **Not shippable until #61 is fixed** — see *Ship blockers*. |
