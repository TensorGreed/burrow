...
| **#180**, **#181**, **#182**, **#183** | Four redactions that return `Ok` over a secret still on the page, because burrow and PDFium read one file two ways | Blocks **the redaction tool reaching the site**, and nothing else. **It does not block building the binding (#137).** Found by the #166 security reviews, each measured end to end with PDFium reading the secret off burrow's output: a `/DescendantFonts` item that is a stream (#180); indirect array items read as two numbers, in `/Widths` and `/CropBox` (#181); `/LastChar` ignored (#182); and a `null` `/Resources` or `/Rotate`, which PDFium reads as present and qpdf drops when it parses (#183). **An operation returning `Ok` over the secret is the one outcome the tool exists to prevent**, which is why these are blockers rather than fidelity work. #183's remedy is decided (2026-09-24): a pre-scan of the input bytes for null-valued `/Resources` and `/Rotate`, refused before the operation runs. It is not PDFium in the operation, which ADR 0029 rejects: every qpdf-based check is blind to a null by construction, so the remedy has to sit before qpdf. Discharged when each has a fixture refused by name, with a near-miss that is not. |
| **#199** | Decoded page content is freed without being wiped on the web path | Blocks **the redaction tool reaching the site**, and nothing else. Redaction reads a page's content **decoded**, and that is the text being redacted. On the web, the caller-owned buffers qpdf returns for decoded content are freed unwiped (`__burrow_qpdf_oh_page_content` and `__burrow_qpdf_oh_stream_data` in `bridge-qpdf.js`), qpdf's object cache is released unwiped at cleanup, and the Rust side's copies are dropped rather than zeroed: the policy's decoded `Vec`s (web redaction is #200, not yet on main) and the input itself, which `bindings/burrow-wasm` drops as a plain `Box<[u8]>`; and in the JS heap, the `Uint8Array` copies the bridge makes of decoded content and the worker makes of the input, released to the garbage collector with their bytes intact. A worker serves many documents in one session, so a freed buffer stays in its heap's free list, readable by whatever allocates next. That is **the secret persisting in the worker after the operation that removed it from the document**. `Session::drop` already wipes the *qpdf-heap* copy of the input for exactly this reason; nothing else on this path is. No read-back path has been found, so it blocks on the posture rather than on a demonstrated leak. **Split is knowingly not blocked by it**, although its pruning reads decoded content through the same two bridge calls: a split output keeps the pages it read, so the decoded text is content the person kept, not a secret the operation removed. Discharged in two halves, because only one of them can be tested. **The wasm heaps, by test**: a canary drawn in an uncompressed content stream is absent, after a redaction and before the worker is reused, from both wasm heaps (the qpdf module's and burrow's). That means wiping what the code can reach, and recycling the worker for qpdf's internal cache, which the C API cannot reach. **The JS heap, by invariant**, because a test cannot inspect it reliably: every `ArrayBuffer` or typed array holding the input or decoded content is zeroed before it is released, and decoded content never becomes a JS string, since a string cannot be wiped. The review of any change to the worker, the bridge or the binding checks that invariant, which `apps/web/CLAUDE.md` states where reviewers read it, and the row stays until that review has been done on the path as it ships. |

No row blocks further M1 development. Each blocks **the deployment it is scoped to** —
M3/M4 for #62, the redaction route for #125, #180–#183 and #199, per their own text — which is the distinction worth
...
| [M1](#m1--core-operations-and-the-web-app) | Merge, split, rotate, reorder, compress — core + web | **shipped, with two commitments unmet.** All five operations ship, core and web, and the site is live at `https://notonlypdf.com`. **Signed releases were listed under *Before M1 ships* and are still not done; SBOM generation is done (2026-09-18)**; the corpus tooling is still stubs. Their state and where they now sit is recorded in [*Before M1 ships*](#before-m1-ships--three-commitments-that-were-not-met) rather than dropped. #61 discharged by ADR 0022 — see *Ship blockers*. |
...
**ships**, pointing the other way. `compress` has since run the same lifecycle — held while the
operation existed only natively, then flipped by the change that gave `WebQpdf` its
`impl DocumentCompressor` — so the test's held list is now **empty** and every one of the five
operations is asserted to ship. An empty held list is the end of that check's useful life as a
gate and it is kept as a shape rather than removed: the list is what a sixth operation is added
to on the day its core lands and its route does not.
...
| `compress` | Output is never larger than the input; page count and page dimensions are unchanged; text remains extractable. **All three hold, and the first is carried by the TYPE rather than by a check**: `compress` returns an `Outcome`, and the only variant carrying a document is the one where it is smaller — there is no code path that returns a larger one. `the_outcome_and_the_bytes_never_disagree` asserts the two halves cannot contradict each other. *Never worse* is not a nicety: **3.4%** of qpdf's own 618-file corpus grows strictly under the lever and a further 5.3% is left byte-identical ([spike 0005](spikes/0005-what-qpdf-alone-compresses.md)). The outcome carries **both sizes and not the input bytes** — a copy of the input on every call would buy nothing, and a person handed back a file of exactly the size they supplied has been told nothing ([ADR 0025](adr/0025-what-compress-does-and-what-it-refuses-to-do.md) §3). **v1 is lossless by construction**: the engine sets one storage lever and touches no content, which is what makes *text remains extractable* true without a test having to establish it. [Spike 0005](spikes/0005-what-qpdf-alone-compresses.md) is why there is one lever — four of the five qpdf's C API exposes are already the writer's own defaults, so every other operation has been emitting them since `merge` shipped, and qpdf has **no** duplicate-object removal at any setting. What it is worth varies by two orders of magnitude and no single figure describes it: **85.6%** on a form, 12.0% on a text report, **0.15%** on a scan, 0.12% on a photo-heavy document, median 16.0%. **The documents people most want to compress are the ones it cannot help**, which is the page's obligation to say before the upload rather than after. ADR 0022's witness is `Expected::Compressed`, a new variant rather than a reuse of `Rotated` — the predicate is the same and the *residue* is not, and it is the largest of any variant's, so `compress_keeps_everything.rs` carries the rest via `assert_nothing_lost`. `fuzz/fuzz_targets/compress.rs` is the only target reaching qpdf's object-stream writer; it must be run **seeded**, because an invalid PDF is refused at `open` and never reaches the writer at all. **`/compress-pdf` ships.** `qpdf_set_object_stream_mode` stood on `engines/qpdf-not-exported.toml` for exactly one pull request — declared natively by the seam, exported by the change that gave `WebQpdf` its `impl DocumentCompressor` — which is that file's stated lifecycle running forwards for the third time. The **page** carries the part of this operation no test can: the wasm `Reply` returns both byte counts rather than a document alone, so a file that did not shrink is a sentence about the file rather than a second round trip or a silently identical download, and `/compress-pdf` explains why a scan will not shrink **before** anything is chosen ([ADR 0025](adr/0025-what-compress-does-and-what-it-refuses-to-do.md) §5) rather than reporting 0.15% and leaving a person to conclude the tool is broken. `src/components/compress-messages.ts` holds the three outcomes — `smaller`, `negligible` (under 1%), `unchanged` — and none of the three is rendered in `--refuse`: a document that was already efficiently stored is not a refusal and not a limit. `apps/web/src/reply-shape.test.ts` is the control the wasm getters earned: two of them shipped without a `js_name`, the worker read `undefined`, and **every** operation failed — so the JS names in `bindings/burrow-wasm/src/lib.rs` are now compared against the fields `apps/web/src/worker/main.js` actually reads. |
...
- **Page thumbnails are #57**, and its first piece has landed: **PDFium loads on demand**, in a
  ...
  **What is still open there, stated because it is a real limit rather than a detail:**
  `FPDF_LoadPage` is uninterruptible and costs 3.5 s and 1,765 MiB on a 1.3 MB page of stroked
  paths. The render itself is no longer unbounded — PDFium's progressive API yields about every
  hundred paths, longest slice 0.7 ms across 100,002 of them — so a deadline now ends a runaway
  render with a typed refusal instead of a terminated worker. The load phase has no checkpoint
  and cannot get one; the only lever is refusing to enter it, which
  `estimate::before_page_load` does. ADR 0027's 2026-09-17 amendments have the figures, and
  [#103](https://github.com/TensorGreed/burrow/issues/103) tracks the residual.

  **The UI landed with it.** `/split-pdf` and `/rotate-pdf` show page pictures, drawn when you
  scroll to them and released when the live window is full; the differential corpus gained
  `Operation::Render` and a fixture with ink on it, because every other well-formed fixture is
  structure and would have compared a blank page against a blank page. `/reorder-pdf` does not:
  a strip you can only look at would show somebody the problem without helping them solve it,
...
done.** *[Amended 2026-09-18: one of the three — SBOM generation — has since been done; see the
`DONE` row below. The sentence is left standing rather than rewritten because it was true of the
ship, and the ship is what this section records.]* Recorded here rather than quietly deleted, because they were real commitments and the
...
| | state |
|---|---|
| **SBOM generation** (CycloneDX, Rust crates *and* the vendored C/C++ engines) | **DONE 2026-09-18.** `sbom/burrow.cdx.json`, 78 components — 55 crates from `cargo metadata --locked` and 23 native components from `engines/licenses.toml`. Generated by `tools/make-sbom.py`, verified in CI by `--check`. |
| **Signed releases** (cosign keyless/OIDC over artifacts and the SBOM) | **PARTIAL, 2026-09-18.** The *web payload* is signed on demand by `deploy.yml`'s `sign` job — a sibling of `publish` consuming the same `production-dist` artifact, so the signed bytes are the deployed bytes; cosign keyless, `contents: write` + `id-token: write`, and no Cloudflare credential in that job. Tagged `deploy-YYYY-MM-DD-<short-sha>`. What is NOT done is the M3/M4 case — binaries a person installs — and the core crates. See ADR 0024's 2026-09-18 amendment. |
| **Enable `.github/workflows/release.yml`** | still a stub: `workflow_dispatch` only, `exit 1` fast, every real job behind `if: false` |

#### The SBOM was done anyway, 2026-09-18, reversing the decision below

**The reasoning below is kept as written, and it was overtaken rather than refuted.** It says
this work "lands there, together" with M3/M4 rather than being "rushed now for a milestone that
cannot use it" — and that argument is about *signing*, which needs an artifact a person
downloads. The SBOM half needs no artifact: it answers *what went into the build*, which the
paragraph at the end of this section already calls "the part that answers that, and it is still
owed". It is no longer owed.

**What was built, and why it is not just a generated file.** A CycloneDX document nobody reads
has the same shape as a check that examines nothing, and this repository has measured four of
those. So `tools/make-sbom.py --check` is verified rather than merely generated — stated precisely,
because a first draft of this paragraph claimed three mutually cross-checking sources and both
reviews measured that only one of the three comparisons could fire:

1. `cargo metadata --locked` — every Rust crate and the licence Cargo records;
2. `engines/licenses.toml` — the native components, which Cargo cannot see at all;
3. **`tools/detect-engine-components.py` — what symbol inspection finds inside the shipped
   binaries.**

(1) and (2) are *inputs*. They describe disjoint sets and cannot contradict each other; what
binds them is that the document is regenerated on every check and diffed against the committed
one, so neither can move without the diff naming what changed. **(3) is the one independent
source, and it can contradict both.**

The third is what makes it more than a restatement of hand-maintained files. HarfBuzz was linked
into PDFium through an entire ADR cycle with no manifest entry, and ICU sat at `linked = false`
while 489 of its symbols were in the binary. `tools/test-make-sbom.sh` reproduces that exact
case: with the HarfBuzz entry removed, the checker names it in every artifact it is detected in
— two of them on a dev box (`libpdfium.so` and `pdfium.wasm`; it is not in `qpdf.wasm`, and
three is the number of artifacts *scanned*), and **one in CI**, whose `test` job builds the
native engines and not the wasm ones. The tool prints each artifact by path rather than a bare
count, so a legitimate 1 is distinguishable from a broken 1; that ambiguity is in CLAUDE.md
against this very tool.

It runs in the CI job that *has* the vendor tree, and **fails** when there is nothing to scan.
Two reviews measured the first version passing over exactly that: with the tree fully present
and every fingerprint made inert, `--check` printed *"the vendor tree is absent"* and exited 0 —
both halves false, and the verdict line then claimed agreement with binaries nothing had
compared. The tool now runs the detector's probe gate itself (importing a module skips its
`main()`, which is where that gate lived), counts artifacts separately from labels, and gates a
native artifact's detected set against the manifest's `linked_in` for exact equality.

**No new dependency.** `cargo-cyclonedx` was not needed — `cargo metadata` already carries
everything, and adding a tool to describe your supply chain is itself a supply-chain change.

**Signing and `release.yml` are still not done**, and the decision below still governs them:
they need an artifact, and there is none.
...
*[Amended 2026-09-18. The argument above is kept because it was the reasoning at the time and it
is why the omission was not a defect — but it has been overtaken twice, and the same sentence
was corrected in `release.yml`'s header for the same reason. The SBOM half needed no artifact at
all. The signing half now HAS an artifact: `deploy.yml`'s `sign` job packs the deployed payload
into a tarball on demand and signs it. The tarball is evidence rather than a distribution
channel — nobody has to fetch it to use the site — which is exactly why it was possible to add
without changing how anything ships. What the paragraph still describes correctly is the M3/M4
case, which is the part that remains owed.]*
...
- **M3/M4 (Android, iOS): precondition.** Those milestones *do* produce binaries a person
  installs. Shipping one with no signature and no provenance is a different proposition from
...
part that answers that. *[Amended 2026-09-18: no longer owed — `sbom/burrow.cdx.json` is
committed and CI verifies it. Signing, which is the other half of provenance, still is.]*
